...
test('checkout helper forwards cohort metadata to the three providers and preserves application references', async ({ page, site, mockd }) => {
  ...
  expect(stripeReference).toMatch(/^jl2_/)
  const pageview = stripeReference.slice(4, 40)
  const events = await mockd.events()
  expect(events.some(event => event.n === 'pageview' && event.id === pageview)).toBe(true)
  expect(Buffer.from(stripeReference.slice(41), 'base64url').toString()).toBe('newsletter~email')
  ...
  expect(new URL(hrefs[2]!).searchParams.get('checkout[custom][jelto_pageview]')).toBe(pageview)
  expect(JSON.parse(new URL(hrefs[3]!).searchParams.get('metadata')!)).toEqual({ order: '123', jelto_cohort: 'newsletter~email', jelto_pageview: pageview })
